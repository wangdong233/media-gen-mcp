...
> 🔴 **Channel status (final verdict 2026-09-10)**: the Google account-region gate (L3, `flow.google.com/unsupported-country`) makes this channel **unavailable** — login and session healthy, network-node changes still rejected, no fix timeline. Code and config surface are retained; the section below is kept as contract documentation (full wire contract archived in `doc/flow-api-contract.md`).
...
> **Render browser dependency**: motion videos and 100%-fidelity filter-SVG rendering recommend the **lasso render tier** (deterministic flags built in, auto-reaps after 10 idle minutes, fully isolated from your everyday Chrome) — install once via `npm i -g lasso-mcp` then run `npx -y lasso-mcp render-chrome --ensure`; `MEDIA_GEN_RENDER_MODE=attach` pins the tier (CI/acceptance). Without lasso, filter SVGs fall back to resvg (~92% fidelity) and motion videos return a structured error with fix guidance (the legacy self-managed pool was retired on 2026-09-03).
...
| Cool motion videos / 100%-fidelity filter SVGs (`render_video` / `render_svg` Chrome backend) | Recommended: install the **lasso render tier** — `npm i -g lasso-mcp`, then run `npx -y lasso-mcp render-chrome --ensure` once (auto-reaps after 10 idle minutes) | Works on install; without lasso, filter SVGs fall back to resvg (~92% fidelity) and motion videos return a structured error with fix guidance (the legacy self-managed pool has been retired) |
...
| Generate images via Google Flow (0 credits) / manage Flow assets | **No key needed**: just log into Flow in local Chrome (launch via `lasso launch-chrome`) | Images / upscaling / upload / delete / share / cancel / queries all 0-credit; video bills credits (7–100 per clip). 🔴 Unavailable since 2026-09-10 due to the L3 account-region gate; no fix timeline |
| Generate images / videos via the PixVerse subscription pool (one subscription bundles 25 video + 14 image models) | Needs a PixVerse subscription + a one-time CLI login (`npx pixverse login`; **opt-in channel, never in default chains** — name `provider="pixverse"` (0.22.0: name to use) | Bills subscription credits; every submission passes a **two-phase billing confirm gate** (estimate + confirm token); per-model prices queryable via `list_models` costCatalog (observed > static estimate > unpublished) |
| Generate images (Nano Banana 2) / videos (Omni = the Veo 3.1 family) via the Gemini web app | A local Chrome on CDP 9225 logged into a Google AI subscription (`lasso launch-chrome --port 9225 --idle-ms 0`, log in once in the visible window; **opt-in channel** — name `provider="gemini"` (0.22.0: name to use)) | Consumes the Google AI subscription **compute quota** (5-hour rolling window + weekly cap; a single video measured at ≈15-20% of the window, every submission carries a quota warning); text-to-image / text-to-video MVP (fixed 8s 16:9) |
| Generate images / videos via SiliconFlow (**HTTP API, direct domestic connection — no proxy**) | Register at siliconflow.cn (phone/WeChat/email; the free model needs real-name verification) → create an API key at cloud.siliconflow.cn/account/ak → config `providers.siliconflow.apiKey` (**opt-in channel** — name `provider="siliconflow"`) | **Kolors permanently free** (IPM 2 / IPD 400 ballpark) + the ¥14 signup voucher covers paid models (Z-Image-Turbo ¥0.10/image / Qwen-Image(-Edit) ¥0.30/image / Wan2.2 video ¥2/clip); image-to-image via Qwen-Image-Edit(-2509, ≤3 images); video fixed 5s/720P, no first/last frames; artifact URLs are 1h-TTL — the tool downloads immediately; every paid-model call carries a price warning |
| Generate anime images via PixAI (**10,000 free credits/day; 🔴 measured 2,100 credits/image ≈ 4 images/day**; image only — the API has no video) | Register at pixai.art + **verify your email** (no daily credits without verification) → credentials, pick one: ① browser DevTools → Local Storage → copy `api.pixai.art:token` → config `providers.pixai.token` (most robust) ② `providers.pixai.email/password` (auto recaptcha login, fragile) ③ official API key at profile/edit/api → `providers.pixai.apiKey` (**opt-in channel** — name `provider="pixai"`) | **Positioning = low-volume anime with the cleanest IP** (you own outputs, private, commercial-safe); measured per-image: tsubaki2=2,100 credits (≈4/day), haruka2=4,100 (≈2/day) — the ×4-batch 800cr discount is UI-only, not programmable; daily quota auto-claimed with balance note; image-to-image via public URL only; 13-29s/image; artifacts auto-download immediately |
| Generate images via Cloudflare Workers AI (**10,000 free neurons/day, renewed daily**; image only — zero video models) | Register at dash.cloudflare.com (the free plan suffices, no credit card) → Workers AI page → Use REST API → create an API Token + copy the Account ID → config `providers.cloudflare.apiToken` / `providers.cloudflare.accountId` (**opt-in channel** — name `provider="cloudflare"`) | **10k neurons/day ≈ 173 flux-schnell images or 95 flux-2-klein multi-reference edits**; SDXL duo at $0 Beta; 8 models (Leonardo's lucid/phoenix reachable via API exclusively here); premium tiers warn per call (dev burns 3,750 neurons/image!); Workers Free overage = hard stop at zero cost / Workers Paid auto-bills $0.011 per 1k; outputs belong to you, commercial use OK |
| Use the HF all-capability toolbox (**hf_tts voice-clone TTS / hf_remove_bg background removal / hf_upscale upscaling / hf_lipsync lip-sync**; free ZeroGPU quota) | Same token as the hfspaces channel (free 5min GPU/day); cloning = Chatterbox (MIT, commercial-safe) — provide a reference audio to clone that voice; bg-removal = BiRefNet; upscale = Tile-Upscaler; lip-sync = LatentSync (verify license before commercial use) | Zero local deployment, one endpoint each (**prerequisite: add hfspaces to the enabledProviders whitelist — the factory default does NOT include it**); quota shared with video (upscaling is the hungriest) |
| Generate videos via HF Spaces (**free ZeroGPU quota runs open-source Wan2.2**; video only, works anonymously with no key) | First add hfspaces to enabledProviders in your config (factory default excludes it — not enabled = naming it is rejected); once enabled it works anonymously out of the box (2min GPU/day, shared egress often exhausted); recommended: free huggingface.co account → Settings → Access Tokens → config `providers.hfspaces.token` (**opt-in channel** — name `provider="hfspaces"`) | Free HF account **5min GPU/day ≈3-5 clips** / PRO $9/mo 40min; 4 models: wan22-i2v (≤5s image-to-video) / wan22-relay (**≤10s first+last-frame relay**) / minimax-h3 (**≤14s open-source MiniMax-H3 Turbo: t2v/i2v/first+last-frame + seed — the paid-API crown model at zero cost on quota**) / cogvideox (t2v fallback, 480p no audio); anonymous GPU rejection (SSE data:null) → switch model or add token; outputs are yours (Wan=Apache2.0, commercial-safe) |
| Generate images / videos via ImagineArt (**🔴 free-tier generation is actually plan-walled** — measured 2026-09-23; unlock at Basic $13/mo) | Register at imagine.art → one-time terminal `npx -y @imagineartofficial/mcp@0.10.0 login --no-browser`, copy the URL into a browser to authorize (**opt-in channel** — name `provider="imagineart"` / `provider="hfspaces"`) | 🔴 The official MCP doc claims 100 free credits/day same-pool, but in reality image generation returns "model not in plan" and video explicitly requires a subscription — credits are visible but unspendable; **the channel works once you subscribe to Basic $13/mo (annual $9/mo)** (31 image + 44 video aggregation models; z-image-turbo 5cr/image, wan-2-2 30cr/clip); do not pick this channel unsubscribed |
...
**Channel routing (0.22.0: priority chains REMOVED — selection belongs to the caller)** — channel choice is a business decision (free drafts → paid finals), made at call time via the comparison surface built into each tool's `provider` description:

- **Default = free pool**: omit `provider` → free channels fail over automatically (agnes ↔ zhipu, 60s circuit breaker) — the zero-cost choice for drafts
- **Opt-in channels, name to use** (prerequisite: add them to the enabledProviders whitelist below): `gemini` (subscription-quota high quality) / `pixverse` (subscription-credit many models) / `siliconflow` (direct domestic API: Kolors free + voucher-covered paid line) / `pixai` (anime vertical, 10,000 free credits/day bound to the account, image only) / `cloudflare` (Workers AI, 10,000 free neurons/day renewed daily, image only) / `imagineart` (aggregation fleet, 🔴 free-tier generation plan-walled, usable after $13/mo) / `hfspaces` (open-source Wan2.2 video with zero deployment, free ZeroGPU quota) work when explicitly named (`provider="gemini"` / `provider="pixverse"` / `provider="siliconflow"` / `provider="pixai"` / `provider="cloudflare"` / `provider="imagineart"` / `provider="hfspaces"`) and pin on failure (structured error, never silently substituted); cost safety is bounded by their own mechanisms (gemini: quota warning on every submission; pixverse: two-phase billing confirm gate — first call returns `{needConfirm, estimatedCost, confirmToken}`, re-call with same params + token to submit; tokens live 10 minutes bound to all billing params; siliconflow: price warning on every paid-model call, ¥14 voucher deducted first; pixai: free credit pool + daily-claim notice; cloudflare: per-call neuron warnings on premium tiers, free plans hard-stop at zero cost)
- **Channel enable whitelist (the single switch since 2026-09-24)**: `"enabledProviders": ["agnes", "zhipu"]` (factory default) — **a generation channel not on the list is NOT enabled**: any `provider` naming is structurally rejected at the router (zero network; the error includes enable instructions); enable exactly what you use by adding channel names to the list, e.g. `["agnes","zhipu","siliconflow","cloudflare","hfspaces","pixverse","gemini"]` (under the ordered strategy the list order is the usage order); env `MEDIA_ENABLED_PROVIDERS` comma-separated, same semantics. The vision chain (tesseract/paddle/vlm/glm-vision) is not governed by the whitelist — always available
- **Channel selection strategy `"providerStrategy"`**: `"caller"` (default) — omitting `provider` stays on the free pool with failover; selection belongs to the caller (opt-in channels still need explicit naming); `"ordered"` — when unnamed, cascade down the enabledProviders list: the first configured channel with the required capability serves, failures fall through to the next; billing confirm gates (e.g. pixverse two-phase) apply under every strategy. env `MEDIA_PROVIDER_STRATEGY` same semantics
- 🔴 The old `imageProviderPriority` / `videoProviderPriority` configs are DEPRECATED: read → warning → ignored (safe to delete from config.json); the selection knowledge now lives in the `provider` parameter descriptions of generate_image / create_video (comparison: free pool / gemini quality / pixverse variety), and `list_models` surfaces the routing notes
...
**PixVerse provider (4th generation channel, subscription credit pool)**: spawns the official CLI (`pixverse --json`) against your logged-in subscription pool — one subscription bundles 25 video models + 14 image models. **opt-in semantics (0.22.0)**: never in default routing (omitting `provider` always stays on the free pool) — name `provider="pixverse"` to use and pin; every image/video submission passes the **two-phase billing confirm gate** (same design as Flow: the first call only returns a cost estimate + confirm token; re-call with the same params + token to actually submit; tokens live 10 minutes and are single-use). Per-model prices are queryable in the `list_models` costCatalog with three states (observed ledger > static estimate > unpublished). Config section `"pixverse": { "confirm": true, "confirmTtlMs": 600000, "pinnedVersion": "1.4.3" }`; the CLI version is pinned + self-checked at startup, drift warns loudly. Full details in `doc/PixVerse-provider集成.md`.

**Gemini web provider (5th generation channel, subscription compute quota)**: CDP UI-drives gemini.google.com (the "Create image" mode = Nano Banana 2 / "Create video" = Omni = the Veo 3.1 family), zero API keys — it uses your local Chrome already logged into a Google AI subscription. **opt-in semantics (0.22.0)**: never in default routing (omitting `provider` always stays on the free pool) — name `provider="gemini"` to use and pin.

One-time setup (3 steps): ① `lasso launch-chrome --port 9225 --mode visible --idle-ms 0` ② log into Google in that window (gemini.google.com shows your account badge) ③ `lasso chrome-hide` to send it back to the background; afterwards a hidden relaunch is enough (`--idle-ms 0` prevents idle reaping; the login lives in the profile). Port is configurable (`GEMINI_CDP_PORT` / `providers.gemini.cdpPort`).

Usage:
- Images: `generate_image(prompt="…", provider="gemini")` (fixed model nano-banana-2; text-to-image ONLY — `images`/`aspect`/`size`/`seed`/`quality` are not consumed, each gets a warning; output is JPEG)
- Videos: `create_video(prompt="…", provider="gemini")` (model omni; FIXED 8s / 24fps / 16:9 — off-spec `numFrames`/`frameRate`/`ratio` etc. are ignored with a warning) → returns a pseudo handle; `get_video(taskId=…, provider="gemini")` polls until completed (MP4 direct link auto-downloaded)

Limits & discipline:
- **Billing = subscription compute quota** (5h rolling window + weekly cap, no per-clip credits, no confirm gate): a single video measured at ≈15-20% of the 5h window, every submission carries a quota warning; images cost little. The only quota readout is the web app's Settings → Usage limits; on exhaustion the page error maps to a terminal `[gemini] S400` (wait for the window to refresh; switching VPN does NOT reset it — quota is bound to the account, the IP only affects regional availability)
- 🔴 **Single live session**: one attached page drives one generation at a time — until the previous gemini video is claimed via `get_video`, a new submission is rejected with `[gemini] S303` (the message points at the get_video call; prevents navigating away from an unclaimed artifact and sinking the quota)
