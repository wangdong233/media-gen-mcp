...
| Generate anime images via PixAI (**10,000 free credits/day, account-bound and programmable**; image only — the API has no video) | Register at pixai.art + **verify your email** (no daily credits without verification) → credentials, pick one: ① browser DevTools → Local Storage → copy `api.pixai.art:token` → config `providers.pixai.token` (most robust) ② `providers.pixai.email/password` (auto recaptcha login, fragile) ③ official API key at profile/edit/api → `providers.pixai.apiKey` (**opt-in channel** — name `provider="pixai"`) | **10,000 credits/day ≈ 12 Standard images** (tsubaki2 default = Tsubaki.2 DiT / haruka2 / hoshino2); daily quota auto-claimed with a balance note; image-to-image via public URL only; best IP terms of all channels (you own outputs, private); the ×4-batch half-price is UI-side only — the tool pays per-image; artifacts auto-download immediately (not retained upstream) |
...
| Generate images / videos via ImagineArt (**100 free credits/day on the SAME pool as the web app**; aggregation fleet, free tier = standard models only) | Register at imagine.art → one-time terminal `npx -y @imagineartofficial/mcp@0.10.0 login --no-browser`, copy the URL into a browser to authorize (or config `providers.imagineart.token` for a long-lived token) (**opt-in channel** — name `provider="imagineart"`) | **100 credits/day ≈ 16-20 z-image-turbo images (5cr) or 3 wan-2-2 videos (30cr)**; 31 image + 44 video aggregation models (NB2/Seedream/Seedance/Kling/Veo listed; free tier standard-only, 720p); ⚠️ free outputs are PUBLIC + non-commercial (draft/asset-pool positioning, commercial at $13/mo); seed unsupported; video via CLI pseudo-handle async |
