...
**PixVerse provider (4th generation channel, subscription credit pool)**: spawns the official CLI (`pixverse --json`) against your logged-in subscription pool — one subscription bundles 25 video models + 14 image models. **opt-in red line**: never in any default chain — either name `provider="pixverse"` explicitly or add it to `"imageProviderPriority"` / `"videoProviderPriority"` at your own cost; every image/video submission passes the **two-phase billing confirm gate** (same design as Flow: the first call only returns a cost estimate + confirm token; re-call with the same params + token to actually submit; tokens live 10 minutes and are single-use). Per-model prices are queryable in the `list_models` costCatalog with three states (observed ledger > static estimate > unpublished). Config section `"pixverse": { "confirm": true, "confirmTtlMs": 600000, "pinnedVersion": "1.4.3" }`; the CLI version is pinned + self-checked at startup, drift warns loudly. Full details in `doc/PixVerse-provider集成.md`.

**Gemini web provider (5th generation channel, subscription compute quota)**: CDP UI-drives gemini.google.com (the "Create image" mode = Nano Banana 2 / "Create video" = Omni = the Veo 3.1 family), zero API keys — it uses your local Chrome already logged into a Google AI subscription. **opt-in red line**: never in default chains — name `provider="gemini"` explicitly or add it to a priority chain.
