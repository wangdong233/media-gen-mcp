...
**Gemini web provider (5th generation channel, subscription compute quota)**: CDP UI-drives gemini.google.com (the "Create image" mode = Nano Banana 2 / "Create video" = Omni), zero API keys — it uses your local Chrome already logged into a Google AI subscription (`lasso launch-chrome --port 9225 --idle-ms 0`; log in once in a visible window the first time). **opt-in red line**: never in default chains — name `provider="gemini"` explicitly or add it to a priority chain. Billing is the subscription **compute quota** (5h rolling window + weekly cap, no per-clip credits, no confirm gate — every video submission carries a quota warning). Outputs: images as JPEG (page-side canvas capture), videos as MP4 direct links; video submission returns a pseudo handle, `get_video` polls for the artifact. Port is configurable (`GEMINI_CDP_PORT` / `providers.gemini.cdpPort`). MVP boundary: text-to-image / text-to-video (image-to-image and aspect/style parameterization not wired). Full research in `doc/Gemini渠道调研-2026-09-22.md`.
