...
### Google Flow Provider (split into the standalone flow-mcp package)

The Google Flow provider (Veo 3.1 / Nano Banana, driven through your local Chrome login — no API key) was split out of this package in 2026-08 into a standalone MCP: **[flow-mcp](https://github.com/wangdong233/flow-mcp)** — 0-credit image generation / upscaling, credit-billed video (abra / Veo 3.1), the `flow_status` credits & media introspection tool, and `flow_entity` character entities.

Both packages **share the same config file** `~/.media-gen-mcp/config.json` (its `flow` section: `enabled` toggle / `imageRouting` / `videoRouting` routing preference — one file, both packages). The credit red line carries over: Flow video bills credits; flow-mcp defaults to `explicit-only` and never auto-routes.
...
| Generate images via Google Flow (0 credits) / manage Flow assets | Install the standalone [flow-mcp](https://github.com/wangdong233/flow-mcp) package (this provider was split out of this one) | Images / upscaling / asset management all 0-credit; video bills credits (explicit-only by default, never auto-routed) |
...
**Provider Priority Chain (optional — declare an ordered `[head, ...fallback]` list)**: add `imageProviderPriority` / `videoProviderPriority` to `config.json`. When the head fails (rate limit / 5xx), the chain **falls through in order**, invisibly to you:
...
- Passing an explicit `provider` (or a `model` — the model itself declares provider ownership) **pins that provider**: no fallback, never a silent substitution.
- Omitting both keys keeps current behavior (default provider + agnes/zhipu free-tier mutual fallback), zero impact. Env equivalents: `MEDIA_IMAGE_PROVIDER_PRIORITY="agnes,zhipu"` / `MEDIA_VIDEO_PROVIDER_PRIORITY="agnes,zhipu"`.
- The Google Flow provider now lives in the standalone [flow-mcp](https://github.com/wangdong233/flow-mcp) package; both packages share this config file (its `flow` section manages the toggle and routing preference — see that package's README). A `"flow"` entry in this package's chain is dropped with a warning; the rest of the order still applies.
