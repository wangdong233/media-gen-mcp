...
|---|---|---|
| **Диаграммы** `generate_diagram` | «Нарисуй архитектуру: клиент → API gateway → два микросервиса» | Архитектура / последовательность / блок-схема / классы / ER / майндмап через **D2** (DSL) или **Graphviz** (DOT) → SVG |
...
| **Карточки** `generate_card` | «Сделай карточку для шеринга с фоном-**градиентом от фиолетового к синему** и 🚀-эмодзи» | OG / социальные / карточки с цитатами (Satori → PNG, по умолчанию 1200×630, **CJK поддерживается автоматически**, **сплошной/градиентный фон**, **цветные эмодзи**) |
