...
Эти инструменты **не вызывают никакого AI**¹ — Claude генерирует DSL/JSON/LaTeX/fields → локально рендерится в SVG/PNG (вектор, высокое разрешение):
...
| **Формулы** `generate_formula` | «Отрендери эту формулу: `\sum_{i=1}^{n} i = \frac{n(n+1)}{2}`» | LaTeX → SVG (MathJax, глифы встроены, шрифт не нужен) |
| **Карточки** `generate_card` | «Сделай карточку для шеринга с фоном-**градиентом от фиолетового к синему**» | OG / социальные / карточки с цитатами (Satori → PNG, по умолчанию 1200×630, **CJK поддерживается автоматически**, **сплошной/градиентный фон**) |
| **Иконки** `generate_icon` | «Дай мне иконку с логотипом GitHub» | 200k+ иконок по запросу (Iconify, `prefix:name`) |
| **QR-коды** `generate_qrcode` | «Сгенерируй QR-код для https://...» | SVG / PNG (полностью локально, ноль сети) |

> ¹ Всё локально и детерминированно, кроме **иконок** (API Iconify) и **шрифта карты по умолчанию** (загружается с CDN при первом использовании, кэшируется в `~/.media-gen-mcp/fonts/`); передайте `fontPath`, чтобы карта работала полностью офлайн. **CJK карточек**: встроенный Noto Sans SC (офлайн, автоопределение китайского/японского/корейского как запасной) — fontPath не нужен. Диаграммы используют [синтаксис D2](https://d2lang.com), графики — [Vega-Lite](https://vega.github.io/vega-lite), формулы — [LaTeX](https://www.latex-project.org), иконки — на [icon-sets.iconify.design](https://icon-sets.iconify.design) — Claude генерирует исходник автоматически.
