...
Эти инструменты **не вызывают никакого AI и работают офлайн**¹ — Claude генерирует DSL/JSON/LaTeX → локально рендерится в SVG (вектор, высокое разрешение):
...
> ¹ Всё локально и детерминированно, кроме иконок. **Иконки** используют публичный API Iconify по запросу + кэш в памяти (мы не включаем в комплект полный набор на 70МБ, чтобы установка через npx оставалась лёгкой) — единственный инструмент, которому нужна сеть. Диаграммы используют [синтаксис D2](https://d2lang.com), графики — [Vega-Lite](https://vega.github.io/vega-lite), формулы — [LaTeX](https://www.latex-project.org), иконки — на [icon-sets.iconify.design](https://icon-sets.iconify.design) — Claude генерирует исходник автоматически.
