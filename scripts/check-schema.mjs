...
import { getProvider, asVideoProvider, getProviderPriority, defaultHead } from "../dist/providers/registry.js";

// F1(B3):链头真源统一 —— 与 src/index.ts buildTools 同一表达式(getProviderPriority 链头,链即开关,S000 已删 → legacy 默认兜底)
let effProvider = defaultHead("video"); // 2026-09-24 审查 P2-6:与 src/index.ts 运行时同真源(感知 ordered 策略)
try {
...
try {
  ...
  console.log("[G2] 27 工具齐全(pares5 M2 +extract_table/analyze_chart/describe_image;pares6 +list_vision_capabilities/extract_pdf/get_pdf;P0-5 +generate_interactive_diagram;P0-5B +generate_nested_diagram;P0-6 +extract_image_meta;flow +flow_status;轮30 HF 全能力 +hf_tts/hf_remove_bg/hf_upscale/hf_lipsync)");
  eq(tools.map((t) => t.name).sort(), ["analyze_chart", "create_video", "describe_image", "extract_image_meta", "extract_pdf", "extract_table", "extract_text", "flow_status", "generate_card", "generate_chart", "generate_diagram", "generate_formula", "generate_icon", "generate_image", "generate_interactive_diagram", "generate_nested_diagram", "generate_qrcode", "get_pdf", "get_video", "hf_lipsync", "hf_remove_bg", "hf_tts", "hf_upscale", "list_models", "list_vision_capabilities", "render_svg", "render_video"], "27 工具齐全");
