...
| 「この棒グラフをデータポイントに読み取って」 | 構造化 JSON データ |
...
- **画像生成**:Nano Banana Pro / Nano Banana 2(デフォルト)/ Nano Banana 2 Lite。比率は 16:9 / 9:16 / 1:1 / 3:4 / 4:3、seed 再現可、ベース画像による編集 + 参考画像(ベース+参考画像で合計 10 枚まで)に対応
...
1 回の呼び出しで 1 本。長さは 4 / 6 / 8 / 10 秒(veo 系に 10 秒は無し —— 4/6/8 秒または key 内蔵の長さのみ。`_Ns` 接尾キー + 異なる durationSeconds は S301)、比率は 16:9 / 9:16 のみ。モード:テキストから動画(t2v。wire は形状検証のみで provider 経由の実際の送信は未実施)、画像から動画(`image`、アップロードは 0 クレジット)、参考画像から動画(`images`、abra キーは最大 7 枚 / veo キーは 3 枚。実験機能として `audioMediaIds` で flow_status の 30 種プリセット音声を参照音声として同時指定可能)、最初と最後のフレーム(`keyframes`、ちょうど 2 枚)、延長(`videoMediaId`)、編集(`videoMediaId` + 編集指示。wire は確定済みだが実際の送信は未検証 —— レスポンスに警告が付きます)、アップスケール(`videoMediaId`)。生成系キーはすべて 720P 固定(ultra/quality バリアントも同様)で、高解像度はアップスケール経由のみ。

> 一部のキーはティアでロック(2026-08 実測 per-tier 価格):fast の `ultra`/`4s`/`6s` 変種と `low_priority` は ADVANCED 専用(10 / 0 クレジット)。plain fast は逆に ADVANCED では利用不可。lite は ADVANCED で 5 クレジット。現在のティアで使えないキーは送信前に拒否され、全ティアの価格表が付きます(クレジットの無駄なし)。キーごとの最新価格は `flow_status` で確認できます。
...
> 手に入るもの:JSON 構造化データ(棒 / 折れ線 / 円グラフすべて対応)
