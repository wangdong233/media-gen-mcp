...
### Google Flow プロバイダ(独立パッケージ flow-mcp へ分離)

Google Flow プロバイダ(Veo 3.1 / Nano Banana、ローカル Chrome のログインセッション経由 —— API Key 不要)は 2026-08 に本パッケージから分離され、独立した MCP になりました:**[flow-mcp](https://github.com/wangdong233/flow-mcp)** —— 0 クレジットの画像生成 / アップスケール、クレジット課金の動画(abra / Veo 3.1)、クレジット・メディア照会ツール `flow_status`、キャラクターエンティティ `flow_entity`。

両パッケージは**同一の設定ファイル** `~/.media-gen-mcp/config.json` を共有します(flow セグメント:`enabled` オンオフ / `imageRouting` / `videoRouting` のルート偏好 —— 1 つのファイルで両方に効く)。クレジットのレッドラインも引き継ぎ:Flow 動画はクレジットを消費し、flow-mcp はデフォルトで `explicit-only`、絶対に自動ルートしません。
...
| Google Flow で画像生成(0 クレジット)/ Flow 資産の管理 | 独立パッケージ [flow-mcp](https://github.com/wangdong233/flow-mcp) を導入(当プロバイダは本パッケージから分離済み) | 画像 / アップスケール / 資産管理はすべて 0 クレジット。動画はクレジット制(デフォルト explicit-only、自動ルートなし) |
...
**プロバイダ優先チェーン(任意 —— 順序付き `[先頭, ...フォールバック]` リストの宣言)**:`config.json` に `imageProviderPriority` / `videoProviderPriority` を追加。先頭が失敗(レート制限 / 5xx)すると、チェーンは**順番に自動フォールバック**し、利用者は意識しません:
...
- 明示的に `provider` を渡す(または `model` を渡す —— model 自体がプロバイダ帰属の宣言)と**そのプロバイダに固定**:フォールバックなし、黙った差し替えは一切なし。
- 両キー未指定は現行挙動のまま(デフォルトプロバイダ + agnes / zhipu 無料枠の相互バックアップ)、影響ゼロ。env 等価:`MEDIA_IMAGE_PROVIDER_PRIORITY="agnes,zhipu"` / `MEDIA_VIDEO_PROVIDER_PRIORITY="agnes,zhipu"`。
- Google Flow プロバイダは独立パッケージ [flow-mcp](https://github.com/wangdong233/flow-mcp) に移りました。両パッケージはこの設定ファイルを共有(flow セグメントでオンオフとルート偏好を管理 —— 詳細はそちらの README)。本パッケージのチェーンに `"flow"` を書くと警告付きで除外され、残りの順序はそのまま適用されます。
