    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          outDir: { type: "string", description: "Output directory; omit = default ./output under the server start dir (the project that launched the task); change globally via config outDir." },
          provider: { type: "string", default: videoHead, description: "Optional; omit = FREE pool (agnes → zhipu auto-failover, zero cost). Channel choice is YOUR business call: [FREE drafts] 'agnes'/'zhipu' (mid-tier, run-through/animatic friendly); [top quality] 'gemini' = Omni/Veo 3.1 via Google AI subscription compute quota (≈15-20% of the 5h window per 8s clip, quota warning every submit, single live session, FIXED 8s/24fps/16:9); [widest models] 'pixverse' = 25 models, subscription credits, two-phase confirm gate; [DISABLED] 'flow' structurally disabled — always rejected. Naming gemini/pixverse pins the channel; naming a free channel still fails over within the free pool. [open-source video, no key] 'hfspaces' = HF Spaces ZeroGPU: free 2-5min GPU/day runs Wan2.2 i2v/first-last-frame relay with zero deployment — pick for free keyframes-relay video or when all vendor channels are cooldown; anonymous works, token raises quota. [domestic direct] 'siliconflow' = Wan2.2 T2V/I2V ¥2/条 (voucher-first), fixed 5s — pins the channel. Priority-chain config was REMOVED in 0.22.0 — selection lives here, by business intent. Vision tools use a separate provider set (tesseract/paddle/vlm/glm-vision — see list_vision_capabilities)." },
        },
        required: ["prompt"],
      },
    },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  try {
    switch (req.params.name) {
      ...
      case "list_models": {
        // 0.22.0:路由说明(链已废弃;免费池 + 点名即用 + 禁用表三要素)
        return ok({
          providers: listProviders(),
          detail: buildListModelsDetail(optString(a.provider)),
          imageRoutingNote: `image 路由(0.22.0):缺省 = 免费池 ${config.defaultImageProvider}(失败自动容灾互备);opt-in 渠道(gemini/pixverse/siliconflow/pixai/cloudflare/imagineart)显式点名即用且钉死(错误直抛不静默换渠道);免费渠道(agnes/zhipu)显式点名后失败仍带告警回落;disabledProviders 禁用渠道(默认 flow)一律拒绝`,
          videoRoutingNote: `video 路由(0.22.0):缺省 = 免费池 ${config.defaultVideoProvider}(agnes↔zhipu 容灾互备);opt-in 渠道(gemini=Omni 高质量配额制/pixverse=25 模型积分制+确认门/siliconflow=Wan2.2 ¥2/条 固定5s/hfspaces=ZeroGPU 免费开源视频 wan22-i2v/relay/cogvideox)点名即用且钉死;pixai/cloudflare 无视频能力(API 明确不支持);flow 默认禁用(disabledProviders)`, 
        });
      }
