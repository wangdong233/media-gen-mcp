...
function buildTools() {
  ...
  return [
    ...
    },
    {
      ...
      description:
        "Generate a structured diagram (architecture/sequence/flowchart/class/ER/mindmap) from DSL code. Renders locally to SVG (vector, high-res) via D2 engine — no AI, deterministic. Claude generates the D2 DSL; this tool renders it. Example: code='x -> y' produces a simple diagram.",
      inputSchema: {
        type: "object",
        properties: {
          code: { type: "string", description: "DSL source code (D2 syntax). Generated by Claude." },
          engine: { type: "string", enum: ["d2", "mermaid", "graphviz"], default: "d2", description: "Render engine (d2 = WASM bundled, always available)" },
          ...
          theme: { type: "string", description: "Theme (D2 theme name or ID)" },
  ...
  try {
    switch (req.params.name) {
      ...
      case "generate_diagram": {
        ...
        if (!engine) return err(`unknown diagram engine: ${engineName} (P0 only d2)`);
