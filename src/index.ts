  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          videoId: { type: "string", description: "Task id from create_video (agnes/zhipu async handle). provider=flow: pass the mediaId as taskId instead (flow has no separate videoId). provider=pixverse: pass the numeric video_id as taskId. provider=gemini: pass the taskId returned by create_video (pseudo handle, in-process only — claim it before submitting another gemini generation)." },
          taskId: { type: "string", description: "legacy fallback endpoint; provider=flow = mediaId, provider=pixverse = numeric video_id, provider=gemini = the pseudo handle from create_video (task status polling)" },
    ...
    {
      name: "list_models",
      description: "List available AI image/video models and video constraints per provider. Generation providers: agnes / zhipu / flow (DISABLED by default via disabledProviders — dead channel since 2026-09-10; live catalog via `flow_status` when re-enabled) / pixverse (subscription-pool CLI; its per-model capability enums — quality/aspect/duration with on_invalid=adjust — are queried lazily per call, not listed here) / gemini (web-UI CDP channel: nano-banana-2 image / omni video, fixed 8s 24fps 16:9, subscription compute quota); vision providers (tesseract/paddle/vlm/glm-vision) are listed too. Use to discover model names (e.g. cogview-4, agnes-video-v2.0, qwen-image, veo-3.1-lite) and allowed video frame counts before calling generate_image / create_video.",
      inputSchema: {
        type: "object",
        properties: { provider: { type: "string", description: "Optional: filter detail to one provider (agnes/zhipu/pixverse/gemini/tesseract/paddle/vlm/glm-vision; flow = disabled by default, shows a disabled entry); omit = all." } },
      },
    },
    {
    ...
    switch (req.params.name) {
      ...
      case "list_models": {
        ...
        return ok({
          ...
          imageRoutingNote: `image 路由(0.22.0):缺省 = 免费池 ${config.defaultImageProvider}(失败自动容灾互备);opt-in 渠道(gemini/pixverse)显式点名即用且钉死(错误直抛不静默换渠道);免费渠道(agnes/zhipu)显式点名后失败仍带告警回落;disabledProviders 禁用渠道(默认 flow)一律拒绝`, 
          videoRoutingNote: `video 路由(0.22.0):缺省 = 免费池 ${config.defaultVideoProvider}(agnes↔zhipu 容灾互备);opt-in 渠道(gemini=Omni 高质量配额制/pixverse=25 模型积分制+确认门)点名即用且钉死;flow 默认禁用(disabledProviders)`, 
        });
      }
