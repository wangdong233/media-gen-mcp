...
function buildTools() {
  ...
  return [
    ...
    {
      name: "list_models",
      description: "List available AI image/video models and video constraints per provider. Generation providers: agnes / zhipu / siliconflow (SiliconFlow HTTP API: Kolors free image + ¥14 voucher covering paid models + Wan2.2 video ¥2/条; needs apiKey — channelInfo card shows quota/limits/prerequisites/risks) / pixai (PixAI anime channel: FREE 10,000 credits/day bound to account, image only, i2i via public URL — token/email-password/apiKey credentials, channelInfo has the setup paths) / flow (DISABLED by default via disabledProviders — dead channel since 2026-09-10; live catalog via `flow_status` when re-enabled) / pixverse (subscription-pool CLI; its per-model capability enums — quality/aspect/duration with on_invalid=adjust — are queried lazily per call, not listed here) / gemini (web-UI CDP channel: nano-banana-2 image / omni video, fixed 8s 24fps 16:9, subscription compute quota); vision providers (tesseract/paddle/vlm/glm-vision) are listed too. Use to discover model names (e.g. cogview-4, agnes-video-v2.0, qwen-image, veo-3.1-lite) and allowed video frame counts before calling generate_image / create_video.",
      inputSchema: {
        type: "object",
        properties: { provider: { type: "string", description: "Optional: filter detail to one provider (agnes/zhipu/siliconflow/pixai/pixverse/gemini/tesseract/paddle/vlm/glm-vision; flow = disabled by default, shows a disabled entry); omit = all." } },
      },
    },
    {
    ...
    switch (req.params.name) {
      ...
      case "list_models": {
        ...
        return ok({
          ...
          imageRoutingNote: `image 路由(0.22.0):缺省 = 免费池 ${config.defaultImageProvider}(失败自动容灾互备);opt-in 渠道(gemini/pixverse/siliconflow/pixai)显式点名即用且钉死(错误直抛不静默换渠道);免费渠道(agnes/zhipu)显式点名后失败仍带告警回落;disabledProviders 禁用渠道(默认 flow)一律拒绝`,
          videoRoutingNote: `video 路由(0.22.0):缺省 = 免费池 ${config.defaultVideoProvider}(agnes↔zhipu 容灾互备);opt-in 渠道(gemini=Omni 高质量配额制/pixverse=25 模型积分制+确认门/siliconflow=Wan2.2 ¥2/条 固定5s)点名即用且钉死;pixai 无视频能力(API 明确不支持);flow 默认禁用(disabledProviders)`, 
        });
      }
