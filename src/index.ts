    ...
    switch (req.params.name) {
      ...
      case "list_models": {
        ...
        return ok({
          ...
          imageRoutingNote: `image 路由(0.22.0):缺省 = 免费池 ${config.defaultImageProvider}(失败自动容灾互备);opt-in 渠道(gemini/pixverse/siliconflow/pixai/cloudflare)显式点名即用且钉死(错误直抛不静默换渠道);免费渠道(agnes/zhipu)显式点名后失败仍带告警回落;disabledProviders 禁用渠道(默认 flow)一律拒绝`,
          videoRoutingNote: `video 路由(0.22.0):缺省 = 免费池 ${config.defaultVideoProvider}(agnes↔zhipu 容灾互备);opt-in 渠道(gemini=Omni 高质量配额制/pixverse=25 模型积分制+确认门/siliconflow=Wan2.2 ¥2/条 固定5s)点名即用且钉死;pixai/cloudflare 无视频能力(API 明确不支持);flow 默认禁用(disabledProviders)`, 
        });
      }
