...
function buildTools() {
  ...
  return [
    ...
    {
      name: "list_models",
      description: "List available AI image/video models and video constraints per provider. Generation providers: agnes / zhipu / flow (flow's live video key catalog is richer via `flow_status`) / pixverse (subscription-pool CLI; its per-model capability enums — quality/aspect/duration with on_invalid=adjust — are queried lazily per call, not listed here) / gemini (web-UI CDP channel: nano-banana-2 image / omni video, fixed 8s 24fps 16:9, subscription compute quota); vision providers (tesseract/paddle/vlm/glm-vision) are listed too. Use to discover model names (e.g. cogview-4, agnes-video-v2.0, qwen-image, veo-3.1-lite) and allowed video frame counts before calling generate_image / create_video.",
      inputSchema: {
        type: "object",
        properties: { provider: { type: "string", description: "Optional: filter detail to one provider (agnes/zhipu/flow/pixverse/gemini/tesseract/paddle/vlm/glm-vision); omit = all." } },
      },
    },
    {
