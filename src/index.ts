...
function buildTools() {
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        type: "object",
        properties: { provider: { type: "string", description: "Optional: filter detail to one provider (agnes/zhipu/siliconflow/pixai/cloudflare/imagineart/pixverse/gemini/tesseract/paddle/vlm/glm-vision; flow = disabled by default, shows a disabled entry); omit = all." } },
      },
    },
    {
    ...
    switch (req.params.name) {
      ...
      case "list_models": {
        ...
        return ok({
          ...
          videoRoutingNote: `video 路由(0.22.0):缺省 = 免费池 ${config.defaultVideoProvider}(agnes↔zhipu 容灾互备);opt-in 渠道(gemini=Omni 高质量配额制/pixverse=25 模型积分制+确认门/siliconflow=Wan2.2 ¥2/条 固定5s/imagineart=CLI 免费日额 wan-2-2 30cr/条)点名即用且钉死;pixai/cloudflare 无视频能力(API 明确不支持);flow 默认禁用(disabledProviders)`, 
        });
      }
