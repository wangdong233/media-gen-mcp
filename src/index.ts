...
import { getProvider, listProviders, resolveProvider, buildListModelsDetail, buildVisionCapabilitiesDetail, getFallbackProvider, asImageProvider, asVideoProvider, asVisionProvider, defaultHead } from "./providers/registry.js";
...
/**
 * 视频链头名(buildTools 的 videoHead 同一表达式,运行时复用):defaultHead("video")
 * (ordered 策略按 enabledProviders 序;caller = defaultVideoProvider)→ 无 video 能力回落 legacy 默认。取件侧(get_video)与 hint 侧(create_video)共用,
...
function videoChainHeadName(): string {
  // 2026-09-24 审查 P2-6:与运行时 resolveProvider 同真源(defaultHead 感知 ordered 策略),
  // 防 schema default 在 ordered 下与实际路由头不符
  try { return asVideoProvider(getProvider(defaultHead("video"))).name; } catch { return config.defaultVideoProvider; }
}
...
function buildTools() {
  ...
  // 链头 = defaultHead("video")(2026-09-24 审查 P2-6:ordered 策略感知;caller = defaultVideoProvider)。
  // 与 scripts/check-schema.mjs 用同一 defaultHead 真源(能力缺失时双方各自回落,保一致)。
  ...
  return [
    {
      name: "generate_image",
      description:
        "Generate or edit an AI image (text-to-image 文生图/AI画图; or image-to-image 图生图 via `images`) using free models (Agnes AI default, or Zhipu). Output downloads locally and the path is returned; no local rendering libs needed.\n\nWHEN: subject is photographic or illustrated (写实图/插画/概念图/original logo artwork / 原创品牌主视觉); user says 'AI画图 / 文生图 / generate an image of ...' and wants AI-generated pixels.\n\nAVOID:\n- Text-heavy cards / OG images / posters / quote cards / cover images → use `generate_card` instead (deterministic Satori render, no AI variability, same input → same output).\n- An existing brand logo (Iconify 200k+ vector set) → use `generate_icon` instead; this tool only draws ORIGINAL logo artwork.\n\nNEXT: call `list_models` first to discover available model names and size constraints per provider. provider=flow (Google Flow via local Chrome): `aspect` (16:9/9:16/1:1/3:4/4:3) and `seed` are honored exactly; outputs carry `mediaId`+`seed` (re-download via `flow_status(mediaId)`); image generation is " + FLOW_ZERO_CREDIT + "-credit. provider=flow also accepts `images` (image-to-image, live-verified): images[0] = base image (aspect follows the base), images[1..9] = references (base + references cap: 10 total) — each is uploaded to the Flow project first (" + FLOW_ZERO_CREDIT + " credits). Image UPSCALE: model=GEM_PIX_2_UPSAMPLE_2K + images[0] (an existing image mediaId, or a URI to upload first) → 2K upscale, " + FLOW_ZERO_CREDIT + " credits, prompt ignored. provider=pixverse (PixVerse subscription pool via official CLI): 14 image models (gpt-image-2.5-flare default / qwen-image / gemini-3.1-flash NB2 / seedream / kling-image…); `aspect` (up to 11 ratios incl 21:9/5:4) and `quality` (720p/1080p…, a first-class BILLING parameter) honored with per-model capability adsorption; images = references (qwen-image max 3); outputs are direct https URLs. 🔴 2026-09-14 final adjudication: the CLI has NO Relax free pool (Standard has no free whitelist — qwen-image actually bills 5/10cr) → EVERY pixverse image submission goes through the TWO-PHASE BILLING CONFIRM GATE (`confirmToken`, same flow as flow video): first call returns {needConfirm, estimatedCost, confirmToken} WITHOUT submitting; re-call with the same params + token to submit. provider=gemini (Gemini web via local Chrome CDP 9225, opt-in): model `nano-banana-2` (the web app's Create-image mode); text-to-image ONLY (images not wired); aspect/size/seed/quality ignored with warnings; output is JPEG; consumes Google AI subscription compute quota (images are cheap; every call's usage shows in Gemini Settings → Usage limits). provider=siliconflow (SiliconFlow 硅基流动, HTTP API, 国内直连零代理, opt-in): 7 image models — Kwai-Kolors/Kolors (the only FREE model, real-name required) + paid models cheaply covered by the ¥14 signup voucher (Tongyi-MAI/Z-Image-Turbo ¥0.10/张, Qwen/Qwen-Image & Qwen-Image-Edit(-2509, ≤3 reference images) ¥0.30/张, ERNIE-Image-Turbo ¥0.11/张); `size` is passed as image_size verbatim (default 1024x1024); `seed` passed verbatim (n>1 fan-out reuses the same seed per image); response URLs are 1h-TTL → the tool downloads each to data:URI immediately; n>1 = tool-layer fan-out to N independent single-image calls (provider ignores n — batch_size removal announced upstream);「AI 生成」watermark on by default (compliance). Needs config providers.siliconflow.apiKey (channelInfo via list_models has the setup path). provider=pixai (PixAI, anime-vertical, opt-in): FREE 10,000 credits/DAY bound to the account (claim-based, email-verified) ≈ 12 Standard images/day — the only free-programmable anime channel (mode B: account API shares the UI pool); 3 models (tsubaki2 default = Tsubaki.2 DiT / haruka2 / hoshino2, SDXL); i2i via public http(s) URL only (GraphQL token channel); IMAGE ONLY (API has no video); ×4-batch half-price only via UI/direct calls — tool-layer fan-out pays per-image price; artifacts auto-download immediately (not retained upstream). Credentials (config providers.pixai): token (browser DevTools → Local Storage → api.pixai.art:token, most robust) / email+password (auto recaptcha login, fragile reverse-eng) / apiKey (official REST v2; billing undocumented — first use verifies). provider=cloudflare (Cloudflare Workers AI, opt-in, IMAGE ONLY): FREE 10,000 neurons/day renewed daily for every account (00:00 UTC reset) — ≈173 flux-schnell images or 95 flux-2-klein edits/day, plus SDXL duo at $0 Beta; 8 models across two wire families (JSON: flux-schnell / sdxl-lightning / sdxl-base / lucid / phoenix; multipart: flux-2-klein (multi-reference edit, ≤4 images <512×512) / flux-2-dev (premium ≈3,750 neurons/image!) / flux-2-klein-9b); premium tiers carry per-call neuron warnings; Workers Paid plans auto-bill overage $0.011/1k neurons (Free plans hard-stop at 429/3036 → cooldown until UTC midnight). provider=imagineart (ImagineArt via official CLI, opt-in): FREE 100 credits/day on the SAME pool as the web app (official MCP billing doc) ≈16-20 z-image-turbo images/day or 3 wan-2-2 videos/day; aggregation fleet (31 image + 44 video models listed; free tier = standard models only, 720p, PUBLIC outputs, non-commercial); seed unsupported (warned); image+video via `imagine` CLI (one-time `login --no-browser`; pseudo-handle async for video); model names passthrough (z-image-turbo default / nano-banana-2 / qwen-image / seedream-4-5 / imagineart-2-0 / wan-2-2 / seedance-1-5-pro …).\n\nMultilingual triggers: 画像 · imagen · image · Bild · изображение · imagem (ja/es/fr/de/ru/pt).",
      inputSchema: {
        type: "object",
        properties: {
          ...
          model: {
            type: "string",
            description: "Optional; omit to use the provider default. Call list_models to see options. provider=flow: GEM_PIX_2_UPSAMPLE_2K = 2K UPSCALE mode (requires images[0] = an existing image mediaId or a URI; " + FLOW_ZERO_CREDIT + " credits, prompt ignored — a non-empty placeholder prompt is still required by the schema). provider=pixverse: 14 models (gpt-image-2.5-flare / gpt-image-2.5-sunburst / gpt-image-2.0 / gemini-3.1-flash / gemini-3.1-flash-lite / qwen-image / gemini-3.0 / gemini-2.5-flash / seedream-5.0-pro / seedream-5.0-lite / seedream-4.5 / seedream-4.0 / kling-image-o3 / kling-image-v3). Cost per model is in list_models costCatalog (ledger-observed > static estimate; 'unknown' = price not published — first use lands the observed cost in the ledger and subsequent calls show it). provider=gemini: nano-banana-2 (single model, web Create-image mode). provider=siliconflow: 7 models (Kwai-Kolors/Kolors FREE / Tongyi-MAI/Z-Image-Turbo / Tongyi-MAI/Z-Image / Qwen/Qwen-Image / Qwen/Qwen-Image-Edit / Qwen/Qwen-Image-Edit-2509 / Baidu/ERNIE-Image-Turbo) — per-model ¥ prices in list_models costCatalog (static prices, voucher-deducted). provider=pixai: tsubaki2 (default, DiT, best) / haruka2 / hoshino2 (SDXL) — or raw model-version IDs; per-mode credit costs in costCatalog (550-1,750 积分/张). provider=cloudflare: flux-schnell (default) / flux-2-klein (multi-ref edit) / flux-2-dev (premium 3,750 neurons!) / flux-2-klein-9b / sdxl-lightning / sdxl-base ($0 Beta) / lucid / phoenix — neurons per image in costCatalog. provider=imagineart: z-image-turbo (default, 5cr) / flux-dev / chatgpt-image-2 / nano-banana-2 / qwen-image / seedream-4-5 / imagineart-2-0 — credits per image in costCatalog.",
          },
          size: { type: "string", description: "e.g. 1024x1024. Zhipu requires each side 512-2880, multiple of 16, pixels ≤ 2^21 — the tool auto-snaps to a valid size; Agnes accepts free size. provider=flow: size maps to the nearest of 5 aspect ratios (1920x1080→16:9 / 720x1280→9:16 / 1024x1024→1:1 / 768x1024→3:4 / 1024x768→4:3); pass `aspect` for an exact ratio. provider=pixverse: non-default size maps to the nearest of 11 aspect ratios (pixverse has no pixel-size parameter). provider=siliconflow: passed verbatim as image_size; per-model allowed档 differ (Kolors: 1024x1024/960x1280/768x1024/720x1440/720x1280; Qwen-Image: 1328x1328 等 7 档) — off-list sizes error upstream (S300 with the API's own message)." },
          ...
          seed: { type: "number", description: "Seed for reproducible results (provider=flow / provider=pixverse / provider=siliconflow pass-through). Other providers ignore it with a warning." },
          ...
          images: {
            type: "array",
            items: { type: "string" },
            description: "Image-to-image inputs (public URL / data URI / absolute local file path — read server-side, ≤15MB). Omit for text-to-image. provider=pixverse also accepts numeric asset ids from its asset library. provider=siliconflow: requires an Edit model — Qwen/Qwen-Image-Edit (1 image) or Qwen/Qwen-Image-Edit-2509 (up to 3); other models + images → S302. provider=pixai: GraphQL token channel only, public http(s) URLs only (data:/local files rejected — upload flow not wired); REST apiKey channel has no image-input field. provider=cloudflare: flux-2 family takes up to 4 reference images (each MUST be <512×512 — resize first); SDXL duo takes 1 data:URI (image_b64); other models reject images.",
          },
          ...
          provider: { type: "string", default: config.defaultImageProvider, description: "Optional; omit = FREE pool (agnes → zhipu auto-failover, zero cost). Channel choice is YOUR business call — pick by draft-vs-final intent: [FREE drafts] 'agnes'/'zhipu' (zero cost, mid-tier quality, iteration-friendly; zhipu takes precise pixel sizes); [top quality] 'gemini' = Nano Banana 2 via Google AI subscription compute quota (little quota per image, JPEG, text-to-image only, measured ~16-30s); [widest models] 'pixverse' = 14 models on a PixVerse subscription (qwen-image 5-10cr, NB2 up to ~60cr per image — full prices in list_models costCatalog; two-phase billing confirm gate; queue ~3-30s per model); Channels not in your config's enabledProviders whitelist are NOT enabled (naming them → structured rejection listing enabled ones + how to enable). Naming gemini/pixverse pins that channel (errors surface, no silent substitution); naming a free channel still fails over within the free pool. [domestic direct] 'siliconflow' = SiliconFlow HTTP API, 国内直连零代理: Kolors free image model + ¥14 signup voucher covering paid models (Z-Image-Turbo ¥0.10/张, Qwen-Image ¥0.30/张, Wan2.2 video ¥2/条) — every paid-model call carries a price warning (no confirm gate: prices are floor-level, voucher-first); naming it pins the channel. [anime free] 'pixai' = PixAI free 10,000 credits/day — 🔴 measured 2,100cr/image (tsubaki2) ≈ 4 images/day; PICK FOR low-volume anime with the cleanest IP terms (you own outputs, private; 13-29s/image). [infra free] 'cloudflare' = Workers AI 10,000 neurons/day, image only — ≈173 schnell images/day at 4-5s/image = the highest free volume+speed; SDXL $0 Beta; premium tiers warned. [aggregator, PAID-GATED] 'imagineart' = official CLI wired but 🔴 free-tier generation is plan-walled (measured: image err 1114, video requires subscription) — usable only after Basic $13/mo. Naming any opt-in channel pins it. providerStrategy config: 'caller' (default) = you pick per call (billing/boundary channels always need explicit naming); 'ordered' = omitting provider walks enabledProviders in YOUR config order with failover. Vision tools use a separate provider set (tesseract/paddle/vlm/glm-vision — see list_vision_capabilities)." },
        },
        required: ["prompt"],
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          numFrames: { type: "number", enum: vc.allowedNumFrames, default: vc.defaultNumFrames, description: "Enum = chain-head (default provider) values, re-validated per actual provider after routing (off-list snaps with a warning — check list_models; e.g. zhipu accepts 150/300 @ 30/60fps; Agnes caps by resolution 1080p≤241 / 720p≤441, over-cap clamps). Mutually exclusive with durationSeconds — pass exactly one. provider=flow: prefer durationSeconds (native set = 96/144/192/240 @24fps); duration-embedded keys (e.g. abra_t2v_8s) reject a conflicting numFrames with S301." },
          frameRate: { type: "number", enum: vc.allowedFrameRates, default: vc.defaultFrameRate, description: "Enum = chain-head (default provider) values, re-validated per actual provider after routing (off-list snaps with a warning; e.g. zhipu accepts 30/60)." },
          ...
          outDir: { type: "string", description: "Output directory; omit = default ./output under the server start dir (the project that launched the task); change globally via config outDir." },
          provider: { type: "string", default: videoHead, description: "Optional; omit = routing by providerStrategy: 'caller' (default) = FREE pool (agnes → zhipu auto-failover, zero cost); 'ordered' = walks enabledProviders in YOUR config order with failover (⚠️ may land on a billing channel — confirm gates still apply; see config providerStrategy). Channel choice is YOUR business call: [FREE drafts] 'agnes'/'zhipu' (mid-tier, run-through/animatic friendly); [top quality] 'gemini' = Omni/Veo 3.1 via Google AI subscription compute quota (≈15-20% of the 5h window per 8s clip, quota warning every submit, single live session, FIXED 8s/24fps/16:9); [widest models] 'pixverse' = 25 models, subscription credits, two-phase confirm gate; [dead zone] 'flow' = Google Flow L3 account-region dead zone since 2026-09-10 — NOT in the default enabledProviders whitelist; naming it before enabling → structured rejection with enable instructions (add \"flow\" to ~/.media-gen-mcp/config.json enabledProviders only if the dead zone is resolved). Naming gemini/pixverse pins the channel; naming a free channel still fails over within the free pool. [open-source video, no key] 'hfspaces' = HF Spaces ZeroGPU: free 2-5min GPU/day runs Wan2.2 i2v/first-last-frame relay with zero deployment — pick for free keyframes-relay video or when all vendor channels are cooldown; requires \"hfspaces\" in enabledProviders (factory default does NOT include it); anonymous works, token raises quota. [domestic direct] 'siliconflow' = Wan2.2 T2V/I2V ¥2/条 (voucher-first), fixed 5s — pins the channel. Channels not in your config's enabledProviders whitelist are NOT enabled (naming them → structured rejection listing enabled ones + how to enable). Priority-chain config was REMOVED in 0.22.0 — selection lives here, by business intent. Vision tools use a separate provider set (tesseract/paddle/vlm/glm-vision — see list_vision_capabilities)." },
        },
        required: ["prompt"],
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          provider: { type: "string", default: videoHead, description: "Provider at task creation; omit = FREE pool (agnes → zhipu auto-failover, zero cost). Channel choice is YOUR business call: [FREE drafts] 'agnes'/'zhipu' (zero cost, mid-tier; good for flow run-throughs and animatics); [top quality] 'gemini' = Omni (Veo 3.1 family) via Google AI subscription compute quota — measured ≈15-20% of the 5h quota window per 8s clip, every submission carries a quota warning, single live session (S303); FIXED 8s/24fps/16:9, ~48s generation; [widest models] 'pixverse' = 25 models on a PixVerse subscription (v6 ~9-23cr/s per clip tier — costCatalog for full prices; two-phase billing confirm gate; 720p cap on Standard tier); Channels outside enabledProviders are rejected at the router (enable-by-config model). create_video responses carry provider_used; pass it back here to poll the right backend. [domestic direct] 'siliconflow' = poll by requestId (results held 10min only — claim promptly; URL 1h TTL, tool downloads to data:URI)." },
        },
      },
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          deleteMediaIds: { type: "array", items: { type: "string" }, description: "Batch-DELETE these project media (" + FLOW_ZERO_CREDIT + " credits, IRREVERSIBLE 不可恢复; POST /v1/flow:batchDeleteAssets). Any unknown id → the whole batch is refused (S400, nothing deleted). Mutually exclusive with mediaId/shareMediaIds/cancelMediaIds. Use to keep the polling payload small." },
          shareMediaIds: { type: "array", items: { type: "string" }, description: "Create PUBLIC share links for these project media (" + FLOW_ZERO_CREDIT + " credits; tRPC flow.share.shareMedia → mediaShareId). Returns shareUrl per media: https://labs.google/fx/tools/flow/shared/{image|video}/<mediaShareId> (prompt included). Mutually exclusive with mediaId/deleteMediaIds/cancelMediaIds." },
          cancelMediaIds: { type: "array", items: { type: "string" }, description: "CANCEL in-flight VIDEO generations for these mediaIds (" + FLOW_ZERO_CREDIT + " credits; POST /v1/flowMedia:cancelGeneration body {mediaId}). Only in_progress media are submitted — completed/failed are reported as notCancelable; status re-checked after (expect MEDIA_GENERATION_STATUS_CANCELED). LIVE-VERIFIED BOUNDARY (2026-08-27): cancel is BEST-EFFORT, mostly theoretical — (a) images are NOT cancelable (404; bundle wires the video queue only); (b) canceling ~2s after submission → 400 FAILED_PRECONDITION (both abra & veo families live-observed); (c) generation is fast (abra 4s ≈17s, veo lite ≈45s wall-clock), so a retry usually finds the media already SUCCESSFUL → notCancelable. Try it only when you catch a wrong submission within seconds. Mutually exclusive with mediaId/deleteMediaIds/shareMediaIds." },
    ...
    {
      name: "list_models",
      description: "List available AI image/video models and video constraints per provider. Generation providers: agnes / zhipu / siliconflow (SiliconFlow HTTP API: Kolors free image + ¥14 voucher covering paid models + Wan2.2 video ¥2/条; needs apiKey — channelInfo card shows quota/limits/prerequisites/risks) / pixai (PixAI anime channel: FREE 10,000 credits/day bound to account, image only, i2i via public URL — token/email-password/apiKey credentials, channelInfo has the setup paths) / cloudflare (Workers AI: FREE 10,000 neurons/day, image only, flux-2 multi-ref edit + SDXL $0 Beta — API Token+Account ID credentials) / imagineart (ImagineArt official CLI: FREE 100 credits/day SAME-POOL with the web app, image+video, free outputs public/non-commercial — one-time `login --no-browser`) / hfspaces (HF Spaces ZeroGPU: open-source video/image/TTS/matting/upscale/lipsync — token optional, raises quota) / flow (dead channel since 2026-09-10 — not in the default whitelist) / pixverse (subscription-pool CLI; its per-model capability enums — quality/aspect/duration with on_invalid=adjust — are queried lazily per call, not listed here) / gemini (web-UI CDP channel: nano-banana-2 image / omni video, fixed 8s 24fps 16:9, subscription compute quota); vision providers (tesseract/paddle/vlm/glm-vision) are listed too. Use to discover model names (e.g. cogview-4, agnes-video-v2.0, qwen-image, veo-3.1-lite) and allowed video frame counts before calling generate_image / create_video.",
      inputSchema: {
        type: "object",
        properties: { provider: { type: "string", description: "Optional: filter detail to one provider (agnes/zhipu/siliconflow/pixai/cloudflare/imagineart/hfspaces/pixverse/gemini/tesseract/paddle/vlm/glm-vision; flow = not in the default whitelist, shows a not-enabled entry); omit = all." } },
      },
    },
    {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          diagramType: { type: "string", description: "Currently ignored — diagram type is determined by DSL syntax (e.g. D2 shape: sequence_diagram). Passing it emits a warning. Reserved for future use." },
          theme: { type: "string", description: "D2 only; named: 'default'(0)/'neutral'(1), or numeric themeID 0-300 (e.g. 200); unknown names / out-of-range IDs error clearly. The graphviz engine ignores theme with a runtime warning" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          width: { type: "number", description: "Target pixel width for PNG (default ~scale x modules; >=300 recommended for print posters). PNG only — SVG is vector with no fixed pixel width (passing it with format=svg emits a warning + is ignored)." },
          name: { type: "string", description: "Output filename (without extension)" },
          outDir: { type: "string", description: "Output directory, default ./output under the server start dir (the project that launched the task); change globally via config outDir" },
        },
        required: ["text"],
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          width: { type: "number", description: "Target pixel width for PNG (default 600); SVG output ignores it (with a warning)" },
          color: { type: "string", description: "Foreground color (default black)" },
          background: { type: "string", description: "PNG background (default #ffffff; set 'transparent' for transparent). SVG output keeps a transparent background and ignores this with a warning. Avoids low-contrast invisible output." },
          name: { type: "string", description: "Output filename (without extension)" },
          outDir: { type: "string", description: "Output directory, default ./output under the server start dir (the project that launched the task); change globally via config outDir" },
        },
        required: ["tex"],
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          background: { type: "string", description: "PNG background (default: white when color=currentColor, transparent otherwise; pass #ffffff/#000000 to override). SVG output ignores this with a warning." },
          format: { type: "string", enum: ["svg", "png"], default: "svg", description: "Output format (svg = vector, default; png = rasterized)" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          blob: { type: "boolean", default: true, description: "hero template only: blurred accent blob behind the title for depth (default true). Passing it on non-hero templates emits a warning + is ignored" },
          quoteStyle: { type: "string", enum: ["top", "flank"], default: "top", description: "quote template only: 'top' = big quote mark above the text (default); 'flank' = large quote marks flank the text left/right on the same line, wrapping it. Passing it on non-quote templates emits a warning + is ignored" },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          width: { type: "number", description: "Target pixel width for PNG (resvg backend uses this; Chrome backend uses SVG intrinsic size x scale and ignores width with a warning; svg pass-through ignores it with a warning). Default: auto-detect." },
          ...
          scale: { type: "number", description: "Retina scale factor for Chrome backend (default 2; only affects Chrome renders — resvg backend and svg pass-through ignore it with a warning)" },
    ...
    },
    {
      name: "hf_tts",
      description:
        "Text-to-speech with optional VOICE CLONING via HF Spaces (Chatterbox, MIT license) — free on ZeroGPU daily quota (hfspaces channel token). PREREQ: hfspaces must be in your enabledProviders whitelist (factory default does NOT include it — first call otherwise returns a structured error with enable instructions; add 'hfspaces' to the enabledProviders array in ~/.media-gen-mcp/config.json). Zero reference audio = Space's built-in sample voice; provide a 5-15s reference audio (http/data URI or local path) to CLONE that voice. Emotional/exaggeration control.\n\nWHEN: 对白配音 / voice clone / 语音合成 where msedge-tts lacks cloning; Chinese supported (multilingual model).\n\nAVOID: plain narration without cloning needs → msedge-tts (faster, no quota); commercial output from F5/E2-TTS spaces (NC license) — Chatterbox is MIT.\n\nNEXT: pair with hf_lipsync to make a talking-head clip.\n\nMultilingual triggers: 配音 · 语音克隆 · clonación de voz · Stimmklon (zh/es/ru/de).",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      name: "hf_upscale",
      description:
        "Upscale/enhance an image (Tile-Upscaler SUPIR family via HF Spaces) — free on ZeroGPU quota. PREREQ: hfspaces must be in your enabledProviders whitelist (factory default does NOT include it — first call otherwise returns a structured error with enable instructions; add 'hfspaces' to the enabledProviders array in ~/.media-gen-mcp/config.json). Tile-based, handles large images.\n\nWHEN: 放大 / 超分 / enhance a low-res image 2-4×.\n\nNOTE: consumes ~20-60s GPU per call (heavier than generation); high multiples eat quota fast.\n\nMultilingual triggers: 放大 · 超分 · upscale · mejorar resolución (zh/es).",
      inputSchema: {
        type: "object",
        properties: {
          image: { type: "string", description: "Input image: http(s)/data: URI or absolute local path (read server-side, ≤15MB)." },
    ...
    },
    {
      name: "hf_lipsync",
      description:
        "Lip-sync a video to new audio (LatentSync via HF Spaces) — re-dub a talking-head clip so the mouth matches the audio. Free on ZeroGPU quota. PREREQ: hfspaces must be in your enabledProviders whitelist (factory default does NOT include it — first call otherwise returns a structured error with enable instructions; add 'hfspaces' to the enabledProviders array in ~/.media-gen-mcp/config.json). 🔴 Inputs must be PUBLIC http(s) URLs (video + audio) — local files are not wired (gradio upload not implemented).\n\nWHEN: 换配音对口型 / re-dub a talking clip (e.g. pair with hf_tts output hosted anywhere reachable).\n\nNOTE: ~30-60s GPU per call; ByteDance LatentSync license — verify before commercial use.\n\nMultilingual triggers: 对口型 · 唇形同步 · sincronización labial (zh/es).",
      inputSchema: {
        type: "object",
        properties: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          includeRaw: { type: "boolean", default: false, description: "Also return the raw workflow JSON object (default false, structured summary only). ComfyUI images only — A1111 has no JSON workflow (see chunks[].textPreview for its raw parameters text). When the image carries no workflow JSON the request emits a warning + is ignored." },
        },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  ...
  /** 解析 outDir:用户指定 → resolve;否则 config.outDir(默认启动目录 ./output)。 */
    ...
    switch (req.params.name) {
      ...
      case "create_video": {
        ...
        const videoReq = {
          ...
          images: refImages, audioMediaIds, videoMediaId,
      ...
      case "get_video": {
        ...
        // provider 缺省 = 视频链头(与 buildTools/create_video 同一真源:defaultHead[策略感知],
      ...
      case "list_models": {
        // 0.22.0:路由说明(链已废弃;免费池 + 点名即用 + enabledProviders 白名单三要素)
        return ok({
          providers: listProviders(),
          detail: buildListModelsDetail(optString(a.provider)),
          imageRoutingNote: `image 路由(2026-09-24 白名单模型):仅 enabledProviders 名单内渠道可用(未列名=未启用,路由层拒绝+启用指引);缺省 = ${config.providerStrategy}(caller=免费池点名制/ordered=按名单顺序级联);opt-in 渠道显式点名即用且钉死;免费渠道显式点名后失败仍带告警回落`,
          videoRoutingNote: `video 路由(2026-09-24 白名单模型):仅 enabledProviders 名单内渠道可用;缺省 = ${config.providerStrategy}(caller=免费池点名制/ordered=按名单顺序级联);hfspaces=ZeroGPU 免费开源视频(wan22-i2v/relay/minimax-h3 音画/cogvideox);siliconflow=Wan2.2 ¥2/条;pixai/cloudflare 无视频能力`, 
        });
      }
      ...
      case "generate_diagram": {
        ...
        if ("fatal" in checked) return err(checked.fatal.message);
        // B10:引擎级丢弃告警(graphviz 忽略 theme 等)+ 产物守门员告警合并透出
        const diagramWarnings = [...(rendered.warnings ?? []), ...checked.warnings];
        return ok({ engine: engineName, format, local_path: fp, ...(diagramWarnings.length ? { warnings: diagramWarnings } : {}) });
      }
      ...
      case "generate_formula": {
        ...
        if ("fatal" in checked) return err(checked.fatal.message);
        // B10:PNG-only 参数丢弃告警(width/background 在 svg 输出下)+ 守门员告警合并透出
        const formulaWarnings = [...(rendered.warnings ?? []), ...checked.warnings];
        return ok({ format, local_path: fp, ...(formulaWarnings.length ? { warnings: formulaWarnings } : {}) });
      }
      ...
      case "generate_icon": {
        ...
        if ("fatal" in checked) return err(checked.fatal.message);
        // B10:PNG-only 参数丢弃告警(background 在 svg 输出下)+ 守门员告警合并透出
        const iconWarnings = [...(rendered.warnings ?? []), ...checked.warnings];
        return ok({ format, local_path: fp, ...(iconWarnings.length ? { warnings: iconWarnings } : {}) });
      }
      ...
      case "generate_card": {
        ...
        const rendered = await renderCard({
          ...
          // B10 丢弃必告警:保留显式 true/false(原实现把显式 true 塌缩成 undefined,
          // card.ts 无法区分「显式传入」与「未传」,非 hero 模板下 blob 会被静默丢弃)。
          blob: a.blob === true || a.blob === "true" ? true : a.blob === false || a.blob === "false" ? false : undefined,
      ...
      case "render_svg": {
        ...
        if ("fatal" in checked) return err(checked.fatal.message);
        // B10:后端专属参数丢弃告警(width/scale)+ 后端降级告警 + 守门员告警合并透出
        const svgWarnings = [...(rendered.warnings ?? []), ...checked.warnings];
        return ok({
          format,
          backend: rendered.backendUsed,
          ...(rendered.warning ? { warning: rendered.warning } : {}),
          local_path: fp,
          ...(svgWarnings.length ? { warnings: svgWarnings } : {}),
        });
      }
