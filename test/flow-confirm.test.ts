...
describe("计费确认门:两段式(无 token 返挑战 → 带 token 放行)", () => {
  test("第一段:needConfirm 挑战(预估 + 令牌 + TTL + hint),且 stub 零提交(无 /video: POST)", async () => {
    ...
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    assert.ok(c, "应返回挑战");
  ...
  test("第二段:同请求 + 有效 token → undefined 放行(随后 createVideo 到达提交端点)", async () => {
    const { t, p } = newProvider();
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    const pass = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c!.confirmToken);
...
describe("计费确认门:wrong token → [flow] S320", () => {
  test("伪造格式 → S320(提示由第一段返回)", async () => {
    ...
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, "garbage-token"),
  ...
  test("他请求签发的 token(参数变化)→ S320(令牌与「key+预估」绑定)", async () => {
    ...
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_10s" }, c!.confirmToken),
  ...
  test("同模型不同时长(mnemonic + durationSeconds)→ 同样 S320(最终 key 变化)", async () => {
    ...
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v", durationSeconds: 10 }, c!.confirmToken),
      (e: any) => e.code === "S320",
    );
  });
});
...
describe("计费确认门:过期 → [flow] S321(TTL 可配)", () => {
  test("confirmTtlMs=60 → 80ms 后复用 → S321(提示重新获取)", async () => {
    ...
    await sleep(80);
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c!.confirmToken),
...
describe("计费确认门:豁免面", () => {
  test("0 积分提交(veo_3_1_upsampler_1080p)→ undefined 不触发门", async () => {
    const { t, p } = newProvider();
    const r = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_upsampler_1080p", videoMediaId: "vid-src-1" });
    assert.equal(r, undefined);
    assert.ok(!t.calls.some((x) => x.method === "POST" && x.url.includes("/video:")));
  });
  test("flow.videoConfirm=false → 整门关闭(付费 key 也直接 undefined)", async () => {
    const { p } = newProvider({ flowCfg: { videoConfirm: false } });
    assert.equal(await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }), undefined);
  });
  test("非 flow 渠道未实现钩子 → undefined(handler 直提交,零影响)", async () => {
    assert.equal(typeof (reg.getProvider("agnes") as any).beginVideoSubmissionConfirm, "undefined");
    assert.equal(typeof (reg.getProvider("zhipu") as any).beginVideoSubmissionConfirm, "undefined");
  });
});
...
describe("计费确认门:预估来源", () => {
  ...
  test("动态目录新增 key(静态表无价)→ estimatedCost=null 保守仍要求确认", async () => {
    ...
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "future_family_9s" });
...
describe("计费确认门:校验同源早失败", () => {
  test("无 model → S300(与 createVideo 同一真源文案)", async () => {
    const { p } = newProvider();
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x" }),
  ...
  test("形状错(t2v key + image)→ S301 指路 i2v key(先于挑战)", async () => {
    const { p } = newProvider();
    await assert.rejects(
      () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s", image: "https://example.com/a.png" }),
  ...
  test("ratio 非 16:9/9:16(generation key)→ S301 先于挑战(审计 A-02:不让用户确认注定失败的请求)", async () => {
    ...
    for (const bad of ["1:1", "4:3", "3:4"]) {
      await assert.rejects(
        () => p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s", ratio: bad }),
        (e: any) => e.code === "S301" && e.message.includes("16:9 / 9:16"),
        `ratio=${bad} 应在确认门第一段即 S301`,
      );
    }
    // 合法值仍返挑战(门不被新校验误伤)
    const ok = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s", ratio: "9:16" });
  ...
  test("ratio 校验豁免 extension/upsampler(方向继承源视频)", async () => {
    ...
    const r = await p.beginVideoSubmissionConfirm({ prompt: "placeholder", model: "veo_3_1_upsampler_1080p", videoMediaId: "01234567-89ab-cdef-89ab-cdef01234567", ratio: "1:1" });
...
describe("确认令牌单次消费(B2-high:同一令牌二次提交 → S320 已使用)", () => {
  test("同 token 第二次校验 → S320 已使用(单次消费语义,防重复扣积分)", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    assert.ok(c1?.confirmToken, "第一段应返回令牌");
    const pass = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c1.confirmToken);
    assert.equal(pass, undefined, "首次带 token 应放行");
    await assert.rejects(
      p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c1.confirmToken),
  ...
  test("重取新 token 可正常放行(单次消费不锁死流程)", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c1!.confirmToken!);
    const c2 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" });
    const pass2 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_t2v_8s" }, c2!.confirmToken!);
...
describe("digest 绑定 prompt(F4:参数变化令牌失效)", () => {
  test("同 key 同价但 prompt 变化 → S320 不符", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "original", model: "abra_t2v_8s" });
    await assert.rejects(
      p.beginVideoSubmissionConfirm({ prompt: "tampered", model: "abra_t2v_8s" }, c1!.confirmToken!),
...
describe("digest 绑定输入引用(三审 finding-3:确认后换输入引用不能复用令牌)", () => {  test("同 key 同价但 videoMediaId 变化(extension)→ S320 不符", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_extension_lite", videoMediaId: "vid-src-1" });
    await assert.rejects(
      p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_extension_lite", videoMediaId: "vid-src-2" }, c1!.confirmToken!),
      (e: any) => /\[flow\] S320/.test(e.message) && /不符/.test(e.message),
  ...
  test("images 参考集变化(r2v 增删参考图)→ S320 不符", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_r2v_8s", images: ["https://example.com/a.png"] });
    await assert.rejects(
      p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_r2v_8s", images: ["https://example.com/a.png", "https://example.com/b.png"] }, c1!.confirmToken!),
      (e: any) => e.code === "S320",
      "参考图集合变化必须使令牌失效",
    );
  });
  test("keyframes 变化(interpolation 换首尾帧)→ S320 不符", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_interpolation_lite", keyframes: ["https://example.com/first.png", "https://example.com/last.png"] });
    await assert.rejects(
      p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_interpolation_lite", keyframes: ["https://example.com/first2.png", "https://example.com/last.png"] }, c1!.confirmToken!),
      (e: any) => e.code === "S320",
      "首尾帧变化必须使令牌失效",
    );
  });
  test("参考图仅顺序调换(images 是集合,排序后入摘)→ 令牌仍有效(语义等价不误杀)", async () => {
    const { p } = newProvider();
    const c1 = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_r2v_8s", images: ["https://example.com/a.png", "https://example.com/b.png"] });
    const pass = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "abra_r2v_8s", images: ["https://example.com/b.png", "https://example.com/a.png"] }, c1!.confirmToken!);
...
describe("确认挑战透出时长吸附/回落告警(#3)", () => {
  test("veo key + durationSeconds=4(目录无 4s 变体)→ challenge.warnings 含回落说明", async () => {
    const { p } = newProvider();
    const c = await p.beginVideoSubmissionConfirm({
  ...
  test("变体切换路径(t2v_lite + 4s,_4s 变体 ADVANCED 档专属 → stub 报 ADVANCED)→ 挑战携带变体切换告警", async () => {
    ...
    const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_t2v_lite", durationSeconds: 4 });
    assert.ok(c);
...
test("知情摘要完备(对抗 C-gap):upsampler+prompt 挑战即含『不消费 prompt』丢弃告警", async () => {
  ...
  const c = await p.beginVideoSubmissionConfirm({ prompt: "x", model: "veo_3_1_r2v_fast_portrait", images: [PNG_1PX], ratio: "16:9" }); // ratio 与 _portrait 后缀不一致 → shape.warnings
