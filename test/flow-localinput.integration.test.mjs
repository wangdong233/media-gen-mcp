...
describe("本地图片路径输入(日志#16;死端口 CDP,确定性零积分)", { skip: await (async () => await portAlive(DEAD_CDP_PORT))() }, () => {
  ...
  fs.writeFileSync(cfgPath, JSON.stringify({ enabledProviders: ["agnes","zhipu","flow"], providers: { flow: { cdpPort: DEAD_CDP_PORT } } }, null, 2)); // 2026-09-24 白名单启用 flow(测输入管线;死端口零积分)
