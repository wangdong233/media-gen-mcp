...
- **Apagar Flow con un interruptor (puerta dura S000)**: `"flow": { "enabled": false }` de nivel superior en `config.json` — Flow se elimina de ambas cadenas de prioridad (la cadena degrada automáticamente al siguiente proveedor), y cualquier llamada explícita `provider="flow"` / modelo de Flow / `flow_status` / `flow_entity` devuelve un error estructurado `[flow] S000` de canal desactivado (el mensaje incluye la corrección: volver a `true`) — nunca un cambio silencioso de proveedor. Ajuste complementario `"flow": { "toolDeadlineMs": 110000 }`: tope de las operaciones largas de Flow (sondeo de imágenes / envío de vídeo / descargas) para cumplir la regla anti-bloqueo (≤120s por llamada); al agotarse devuelve `[flow] S410` — la operación subyacente NO se cancela; revisa y descarga después con `flow_status(mediaId)`.
