...
**Gestión de activos de Flow (todo 0 créditos)**: además de créditos / estado de medios / descargas, `flow_status` soporta tres operaciones sin coste — `shareMediaIds=[…]` crea enlaces públicos para compartir (como `labs.google/fx/tools/flow/shared/image/<id>`, prompt incluido), `cancelMediaIds=[…]` cancela generaciones de VÍDEO en curso (ojo: los trabajos de imagen no se pueden cancelar) y `deleteMediaIds=[…]` borra medios por lotes. La herramienta
