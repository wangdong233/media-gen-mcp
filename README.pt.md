...
- **Desligar o Flow num interruptor (portão rígido S000)**: `"flow": { "enabled": false }` de nível superior no `config.json` — o Flow é removido de ambas as cadeias de prioridade (a cadeia degrada automaticamente para o próximo provider), e qualquer chamada explícita `provider="flow"` / modelo do Flow / `flow_status` / `flow_entity` devolve um erro estruturado `[flow] S000` de canal desativado (a mensagem traz a correção: voltar a `true`) — nunca uma troca silenciosa de provider. Ajuste complementar `"flow": { "toolDeadlineMs": 110000 }`: teto das operações longas do Flow (polling de imagens / submissão de vídeo / downloads) para cumprir a regra anti-travamento (≤120s por chamada); ao estourar devolve `[flow] S410` — a operação subjacente NÃO é cancelada; reverifique e baixe depois com `flow_status(mediaId)`.
