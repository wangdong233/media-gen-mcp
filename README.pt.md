...
### Provider Google Flow (separado no pacote independente flow-mcp)

O provider Google Flow (Veo 3.1 / Nano Banana, controlado pela sessão do Chrome local — sem API Key) foi separado deste pacote em 2026-08 e tornou-se um MCP independente: **[flow-mcp](https://github.com/wangdong233/flow-mcp)** — geração / upscale de imagens a 0 créditos, vídeo cobrado em créditos (abra / Veo 3.1), a ferramenta de introspeção `flow_status` (créditos e média) e as entidades de personagem `flow_entity`.

Os dois pacotes **partilham o mesmo ficheiro de configuração** `~/.media-gen-mcp/config.json` (a sua secção `flow`: interruptor `enabled` / preferências de roteamento `imageRouting` / `videoRouting` — um só ficheiro). A linha vermelha dos créditos mantém-se: o vídeo do Flow consome créditos; o flow-mcp usa `explicit-only` por defeito e nunca roteia automaticamente.
...
| Gerar imagens via Google Flow (0 créditos) / gerir ativos do Flow | Instalar o pacote independente [flow-mcp](https://github.com/wangdong233/flow-mcp) (este provider foi separado deste pacote) | Imagens / upscale / gestão de ativos tudo a 0 créditos; o vídeo cobra créditos (explicit-only por defeito, nunca roteado automaticamente) |
...
**Cadeia de prioridade de providers (opcional — declarar uma lista ordenada `[cabeça, ...fallback]`)**: adicione `imageProviderPriority` / `videoProviderPriority` ao `config.json`. Quando a cabeça falha (rate limit / 5xx), a cadeia **cai em ordem**, de forma invisível para você:
...
- Passar um `provider` explícito (ou um `model` — o próprio modelo declara a que provider pertence) **fixa esse provider**: sem fallback, nunca uma substituição silenciosa.
- Omitir ambas as chaves mantém o comportamento atual (provider predefinido + fallback mútuo agnes/zhipu no nível gratuito), impacto zero. Equivalentes env: `MEDIA_IMAGE_PROVIDER_PRIORITY="agnes,zhipu"` / `MEDIA_VIDEO_PROVIDER_PRIORITY="agnes,zhipu"`.
- O provider Google Flow agora vive no pacote independente [flow-mcp](https://github.com/wangdong233/flow-mcp); os dois pacotes partilham este ficheiro de configuração (a secção `flow` gere o interruptor e a preferência de roteamento — ver o README desse pacote). Uma entrada `"flow"` na cadeia deste pacote é descartada com um aviso; a restante ordem continua a aplicar-se.
