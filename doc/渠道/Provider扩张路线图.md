...
| **flow** | 图 + 视频 | 图/上采样 **0 积分**;视频 **7-100 积分/条**(abra 7-20 / veo lite 10 / fast 20 / quality 100) | 本地 Chrome CDP 页面上下文(lasso 9223),无 API key | 高质量视频(veo/abra)。🔴 **2026-09-10 定谳 L3 账号地区门禁不可用**,代码保留,契约存档 `flow-api-contract.md` |
| **pixverse** | 图 + 视频(P1 面:t2v/i2v) | 订阅积分池(**CLI/API 无 Relax 免费池**——Relax 是 Web UI 独有权益,一切提交按计费) | spawn 官方 CLI `pixverse --json`(版本锁 1.4.3,opt-in) | 一个订阅聚合 25 视频 + 14 图像模型;两段式确认门 + 成本台账 + costCatalog 三态价格目录(契约 `PixVerse-provider集成.md`,ADR-4) |
...
- **优先级链**:工具参数点名 > 模态优先级链(`imageProviderPriority` 等,失败按序 fall through)> 全局默认。点名免费渠道(agnes/zhipu)失败仍可带警告回落;点名 opt-in 渠道(flow/pixverse)则**钉定**,错误如实上报不静默换渠道。
...
- **计费确认门**(flow 视频 + pixverse 全模式):预估消耗 >0 积分且无 `confirmToken` → 返回挑战(needConfirm/estimatedCost/confirmToken/TTL),**绝不提交**;带令牌二次调用才放行(令牌与 key+预估+prompt+输入引用绑定,单次消费)。pixverse 侧另加成本台账落账 + `list_models` costCatalog 三态价格目录。
...
新渠道一律沿用现有治理件:优先级链成员化(链即开关)、(若计费)确认门二段式、限流自学习。计费渠道红线:**绝不自动路由到消耗积分/费用的路径**(flow 视频与 pixverse 全模式已按此落地,ADR-2/ADR-4)。🔴 **第 5 家计费渠道入仓前必须先收敛**复制债(F1 确认门 crypto 管线单源 / F2 digest sig 构造器 / F4 模型清单单源),否则判重大偏差(2026-09-14 C 终裁)。

> **判据 1 的在册例外**:PixVerse 为订阅包干制(非低价按次),经用户显式裁决接入——风险面由 opt-in 钉定 + 全模式确认门 + 成本台账三重治理补齐;此例外不构成后续付费渠道的默认先例。
