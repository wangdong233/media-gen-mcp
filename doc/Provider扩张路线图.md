...
- **S000 硬禁用门**:config 顶层 `flow.enabled=false`(仅显式 false)→ 该渠道从链中剔除 + 显式调用返回结构化 S000 错误(2026-08-24 吸收自 flow-mcp,渠道无关通用钩子)。
...
新渠道一律沿用现有治理件:优先级链成员化、S000 硬禁用门(`enabled:false`)、(若计费)确认门二段式、限流自学习。计费渠道红线:**绝不自动路由到消耗积分/费用的路径**(参照 flow 视频确认门设计)。
