...
- **Bilder flow-first**: Flow-Bildgenerierung kostet 0 Credits; läuft Chrome nicht / nicht eingeloggt, fällt es automatisch auf agnes → zhipu zurück (Probes unterliegen einem 60-Sekunden-Circuit-Breaker, keine Retry-Stürme). `provider=flow` pinnt den Provider fest — Fehler werden direkt gemeldet, kein Fallback (Pinning gilt nur für Opt-in-Provider; ein explizit genannter kostenloser Provider (agnes/zhipu) fällt bei Fehler weiterhin mit Warnung durch die Kette).
